...
class BitboardEvaluator:
    ...
    def __init__(self) -> None:
        ...
        for f in range(8):
            if f > 0: self.neighbor_masks[f] |= FILES[f-1]
            if f < 7: self.neighbor_masks[f] |= FILES[f+1]
    ...
    def _init_passed_masks(self) -> None:
        ...
        for sq in range(64):
            ...
            for rr in range(r + 1, 8):
                ...
                if f > 0: w_front |= (1 << chess.square(f - 1, rr))
                if f < 7: w_front |= (1 << chess.square(f + 1, rr))
            ...
            for rr in range(0, r):
                ...
                if f > 0: b_front |= (1 << chess.square(f - 1, rr))
                if f < 7: b_front |= (1 << chess.square(f + 1, rr))
    ...
    def _init_king_masks(self) -> None:
        ...
        for sq in range(64):
            ...
            if r < 7:
                ...
                if f > 0: w_shield |= (1 << chess.square(f-1, r+1))
                if f < 7: w_shield |= (1 << chess.square(f+1, r+1))
    ...
    def evaluate(self, board: chess.Board) -> int:
        ...
        phase_weights = {chess.KNIGHT: 1, chess.BISHOP: 1, chess.ROOK: 2, chess.QUEEN: 4, chess.KING: 0}
        ...
        for pt in [chess.KNIGHT, chess.BISHOP, chess.ROOK, chess.QUEEN, chess.KING]:
            ...
            if table_mg:
                for sq in squares: mg_score += table_mg[sq]
            if table_eg:
                for sq in squares: eg_score += table_eg[sq]
            ...
            if table_mg:
                ...
                for sq in squares: mg_score -= table_mg[chess.square_mirror(sq)]
            if table_eg:
                for sq in squares: eg_score -= table_eg[chess.square_mirror(sq)]
    ...
    def _eval_pawns_bitwise(self, my_pawns: int, opp_pawns: int, color: bool) -> int:
        ...
        for sq in chess.SquareSet(my_pawns):
            ...
            # Passed Check
            passed_mask = self.white_passed_masks[sq] if color == chess.WHITE else self.black_passed_masks[sq]
    ...
    def _eval_king_safety(self, board: chess.Board) -> int:
        ...
        if w_king_sq is not None:
            if w_king_sq == chess.E1:
                can_castle = (board.castling_rights & chess.BB_H1) or (board.castling_rights & chess.BB_A1)
                if not can_castle:
            ...
            if shield_mask:
                pawns_in_shield = (shield_mask & board.pieces_mask(chess.PAWN, chess.WHITE)).bit_count()
        ...
        if b_king_sq is not None:
            if b_king_sq == chess.E8:
                can_castle = (board.castling_rights & chess.BB_H8) or (board.castling_rights & chess.BB_A8)
                if not can_castle:
                    score += LOST_CASTLING_PENALTY
            ...
            if r > 0:
                for df in [-1, 0, 1]:
                    if 0 <= f+df <= 7:
                        ...
                        if not (piece and piece.piece_type == chess.PAWN and piece.color == chess.BLACK):
                            missing += 1
            if missing > 1:
    ...
    def _eval_threats(self, board: chess.Board) -> int:
        ...
        """
        score = 0
        THREAT_WEIGHTS = {chess.PAWN: 10, chess.KNIGHT: 30, chess.BISHOP: 30, 
                          chess.ROOK: 50, chess.QUEEN: 80, chess.KING: 0}
