...
class PlayRequest(BaseModel):
    ...
    user_move: str = Field(
        ..., description="User's move in UCI format (e.g. 'e2e4')."
    )
    depth: int = Field(
        default=DEFAULT_DEPTH,
        ge=1,
...
class BestMoveResponse(BaseModel):
    move: Optional[str] = Field(None, description="Best move in UCI format, or null if game is over.")
    san: Optional[str] = Field(None, description="Best move in SAN notation, or null if game is over.")
    depth: int = Field(..., description="Search depth used.")
    score_cp: int = Field(..., description="Evaluation in centipawns (from White's perspective).")
...
class PlayResponse(BaseModel):
    fen: str = Field(..., description="Board FEN after both moves (or after user move if game ended).")
    engine_move: Optional[str] = Field(None, description="Engine's response in UCI format, or null.")
    engine_san: Optional[str] = Field(None, description="Engine's response in SAN notation, or null.")
    ...
    error: Optional[str] = Field(None, description="Error message if the move was illegal.")
